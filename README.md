...
Plain HTML + CSS + a little JS, no build tools, no server needed. Free to host on GitHub Pages.

All files sit in one flat folder — no subfolders — to keep uploading simple.
...
- `slideshow.js` — powers the multi-image slideshow on product cards
- `images/` — put product photos in here once you have them (see below)
...
Each product card on the Sarees/Lehengas pages has a `.slideshow` block with
2 placeholder color slides and dots underneath — visitors can click a dot,
and it also auto-advances every 4 seconds. This is exactly where your
...
## Uploading to GitHub
Everything is flat now, so uploading is simple: drag in whichever `.html`,
`.css`, or `.js` files changed via "Add file → Upload files" and confirm the
overwrite prompt — no folders to worry about breaking. The one exception is
`images/` — when you add product photos, drag that folder in as a whole so
the photos land inside it rather than loose in the main folder.

## To go live for free (GitHub Pages)
1. Create a free GitHub account and a new repository (e.g. `om-textiles`).
2. Upload all these files into that repository.
